...
class Model(BaseModel):
	...
	# Read lookup table path
	self.lookup_word_fact = kwargs.get('lookup_word_fact', None)
    ...
    def beam_search(self, inputs, beam_size=12, maxlen=50, suppress_unks=False, **kwargs):
	    ...
	    dic_lem = dict()
	    #lookup_word_fact = codecs.open(self.lookup_word_fact, 'r', encoding="utf-8")
		...
		for idx, [cand_h_scores_lem, cand_h_scores_fact] in enumerate(zip(cand_scores_lem, cand_scores_fact)):
			...
			for f in range(beam):
			    ...
			    lem = self.trg_idict.get(word_indices_lem[l], "<unk>")
			    fact = self.trgmult_idict.get(word_indices_fact[f], "<unk>")
			    # Check if we have the lemma in the table
			    if lem in dic_lem.keys():
			        # if we have it, check if the factos are correct for this lemma
				if fact not in dic_lem[lem]:
#				    print ('incorrect factors')
#				    print ('lem=', lem.decode('utf-8').encode('utf-8'), 'fact=', fact)
				    # Go to the next factors according to the probs
				    for fa in range(fact_all,self.n_words_trgmult):
					fact = self.trgmult_idict.get(word_indices_fact_all[fa], "<unk>")
#					print ('lem=', lem.decode('utf-8').encode('utf-8'), 'fact2=', fact)
					if fact in dic_lem[lem]:
					    # Update the candidates with the correct factors
					    # TODO check if this factor is already taken for the same lemma
					    ...
					    fact_all = fa
					    # TODO What can we do when there are less than beam possibilities that are correct?
					    break
			    else:
...
#				print ('WARNING:,'+lem.decode('utf-8').encode('utf-8')+'lemma not in the lookup table')
