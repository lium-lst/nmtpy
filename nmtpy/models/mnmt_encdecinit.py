...
class Model(Attention):
    ...
    def init_params(self):
        ...
        # init_state computation from pool5 image features
        params = get_new_layer('ff')[0](params, prefix='ff_imginit',
                    nin=self.img_dim, nout=self.rnn_dim, scale=self.weight_init)
    ...
    def build(self):
        ...
        init_state = get_new_layer('ff')[1](self.tparams, x_img, prefix='ff_imginit', activ='tanh')
        ...
        proj = get_new_layer(self.enc_type)[1](self.tparams, emb, prefix='encoder',
                                               init_states=init_state, mask=x_mask, layernorm=self.layer_norm)
        ...
        projr = get_new_layer(self.enc_type)[1](self.tparams, embr, prefix='encoder_r',
                                                init_states=init_state, mask=xr_mask, layernorm=self.layer_norm)
    ...
    def build_sampler(self, **kwargs):
        ...
        # encoder
        proj  = get_new_layer(self.enc_type)[1](self.tparams, emb, prefix='encoder', layernorm=self.layer_norm, init_states=init_state)
        projr = get_new_layer(self.enc_type)[1](self.tparams, embr, prefix='encoder_r', layernorm=self.layer_norm, init_states=init_state)
