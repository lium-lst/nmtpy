...
class Model(Attention):
    ...
    def init_params(self):
        ...
        # img features to ctxdim
        params = get_new_layer('ff')[0](params, prefix='ff_imginit',
                    nin=self.img_dim, nout=self.ctx_dim, scale=self.weight_init)
    ...
    def build(self):
        ...
        # Image features
        img_feats = get_new_layer('ff')[1](self.tparams, x_img, prefix='ff_imginit', activ='tanh')

        # Apply dropout
        ctx = dropout(ctx[0], self._trng, self.ctx_dropout, self.use_dropout)

        # Modulate source context with image features
        ctx = ctx * img_feats[None, ...]
