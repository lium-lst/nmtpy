...
############################################################
# Zero-init CGRU, y_emb *= img_feats
############################################################
...
class Model(Attention):
    ...
    def init_params(self):
        ...
        # Multiply image features with target embeddings
        params = get_new_layer('ff')[0](params, prefix='ff_img',
                    nin=self.img_dim, nout=self.embedding_dim, scale=self.weight_init)
    ...
    def build(self):
        ...
        # Transform pool5 features
        img_feats = get_new_layer('ff')[1](self.tparams, x_img, prefix='ff_img', activ='tanh')

        # assume zero-initialized decoder
        init_state = T.alloc(0., n_samples, self.rnn_dim)
    ...
    def build_sampler(self, **kwargs):
        ...
        img_feats = T.TensorType(FLOAT, (True, False))('img_feats')

        # if it's the first word, emb should be all zero and it is indicated by -1
        emb = T.switch(y[:, None] < 0,
                            T.alloc(0., 1, self.tparams[self.trg_emb_name].shape[1]),
                            self.tparams[self.trg_emb_name][y] * img_feats)        # Multiply emb with img_feats
        ...
        inputs = [y, init_state, ctx, img_feats]
