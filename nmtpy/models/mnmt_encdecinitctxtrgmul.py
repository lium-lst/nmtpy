...
class Model(Attention):
    ...
    def build(self):
        ...
        proj = get_new_layer(self.enc_type)[1](self.tparams, emb, prefix='encoder', init_states=init_state, mask=x_mask, layernorm=self.lnorm)

        # word embedding for backward rnn (source)
        embr = dropout(self.tparams['Wemb_enc'][xr.flatten()],
                       self.trng, self.emb_dropout, self.use_dropout)
        embr = embr.reshape([n_timesteps, n_samples, self.embedding_dim])
        projr = get_new_layer(self.enc_type)[1](self.tparams, embr, prefix='encoder_r', init_states=init_state, mask=xr_mask, layernorm=self.lnorm)
    ...
    def build_sampler(self):
        ...
        emb = T.switch(y[:, None] < 0,
                            ...
                            self.tparams['Wemb_dec'][y] * img_ymul)        # Multiply emb with img_ymul
